...
PrSM 5는 일반 `yield`, 어트리뷰트 타깃, 전처리 디렉티브, `ref`/`out`/`vararg`/명명/기본 매개변수, `nameof`, `@burst`, 컨텍스트 교차 `unlisten`, 새 패턴 형식 (관계, 결합자, 위치, 프로퍼티), `with` 표현식, 제네릭 제약, `ref` 로컬, `ref struct`, `stackalloc`, span 슬라이싱, `partial` 선언, 일반화된 nested 선언, discard, 안전 인덱스 표현식을 다루는 42개의 새 오류 코드 (E147–E188)와 5개의 새 경고 코드 (W033–W037)를 도입합니다. 언어 4 경고 W031 (bind 미사용)이 이제 실제로 발생합니다.
...
| E180 | ref struct | `allows ref struct` 없는 제네릭 타입 인자 |
