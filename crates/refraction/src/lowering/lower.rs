...
pub fn lower_file(file: &File) -> CsFile {
    ...
    // file-scoped using alias directive. The lower_decl call above
    // produces a marker class so the file shape stays consistent
    // (header → usings → class), but the meaningful output is the
    // `using Name = Target` line added to the usings list here. C#
    // `using` aliases are file-local; cross-file alias resolution
    // would require a project-wide pass and is tracked separately.
    if let Decl::TypeAlias { name, target, .. } = &file.decl {
        let alias_text = format!("{} = {}", name, lower_type(target));
        if !usings.iter().any(|u| u == &alias_text) {
            usings.push(alias_text);
        }
    }
